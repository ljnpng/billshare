...
const AssignStep: React.FC = () => {
  ...
  const handlePersonToggle = (itemId: string, personId: string) => {
    ...
    const newAssignedTo = item.assignedTo.includes(personId) ? item.assignedTo.filter((id) => id !== personId) : [...item.assignedTo, personId];
  ...
  return (
    <div className="max-w-4xl mx-auto">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                <span className="text-base font-semibold text-gray-900">
                  {t('progressText', {
                    assigned: getAssignedItemsCount(),
                    total: allItems.length,
                  })}
                </span>
              </div>
              {getAssignedItemsCount() === allItems.length && <span className="text-sm text-green-700 font-semibold">{t('allAssigned')}</span>}
            </div>
          </div>
                          ...
                          <div className="text-sm text-gray-600">
                            {item.assignedTo.length > 0 ? (
                              <span className="text-green-600 font-semibold">
                                {t('assigned', {
                                  count: item.assignedTo.length,
                                })}
                              </span>
                            ) : (
                              <span className="text-red-600 font-semibold">{t('unassigned')}</span>
                            )}
                          </div>
                          {item.assignedTo.length > 0 && (
                        ...
                        {people.map((person) => (
                          <button
                            ...
                            className={`p-3 rounded border-2 flex items-center justify-center transition-colors duration-200 ease-in-out ${
                              item.assignedTo.includes(person.id) ? 'border-blue-600 bg-blue-50 text-blue-800' : 'border-gray-300 bg-white hover:border-gray-400'
                            }`}
                          ...
                          >
                            <div className="person-color-sm" style={{ backgroundColor: person.color }} aria-hidden="true" />
      ...
      <div className="flex justify-between gap-3">
        ...
        <button onClick={handleNext} className="btn btn-primary btn-md sm:btn-lg" disabled={getAssignedItemsCount() !== allItems.length} aria-label={t('nextButton')}>
          {t('nextButton')}
        </button>
      </div>
    </div>
